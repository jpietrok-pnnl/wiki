...
(function() {
  ...
  packUnits = function(nums, denoms) {
    var d, j, keep, len, n, ref, ref1, unit, where;
    n = (ref = []).concat.apply(ref, nums);
    d = (ref1 = []).concat.apply(ref1, denoms);
    keep = [];
  ...
  product = function(v) {
    return simplify(v.reduce(function(prod, each) {
      var e, ed, en, p, pd, pn, ref, ref1;
      ref = unpackUnits(prod), p = ref[0], pn = ref[1], pd = ref[2];
      ref1 = unpackUnits(each), e = ref1[0], en = ref1[1], ed = ref1[2];
      return {
        value: p * e,
  ...
  ratio = function(v) {
    var d, dd, dn, n, nd, nn, ref, ref1;
    ref = unpackUnits(v[0]), n = ref[0], nn = ref[1], nd = ref[2];
    ref1 = unpackUnits(v[1]), d = ref1[0], dn = ref1[1], dd = ref1[2];
    return simplify({
      value: n / d,
  ...
  parser = function(lexed) {
    ...
    fact = function() {
      var c, ref;
      c = lexed.shift();
      if ((ref = typeof c) === "number" || ref === "object") {
        return c;
      }
      if (c === "(") {
        c = expr();
  ...
  dispatch = function(state, done) {
    var apply, args, attach, change, color, comment, count, err, hover, input, label, line, list, local, lookup, output, patch, polynomial, previous, ref, ref1, result, s, show, units, unpatched, v, value;
    ...
    try {
      if (args = line.match(/^([0-9.eE-]+) +([\w \.%(){},&\*\/+-]+)$/)) {
        ...
        ref = [apply(args[1], list, args[2]), [], list.length], value = ref[0], list = ref[1], count = ref[2];
        color = '#ddd';
        ...
        ref1 = [apply(args[1], list), [], list.length], value = ref1[0], list = ref1[1], count = ref1[2];
        value = patch(value);
        local[args[1]] = value;
        color = '#ddd';
        hover = args[1] + " of " + count + " numbers\n= " + (asValue(value)) + " " + (printUnits(asUnits(value)));
  ...
  emit = function(div, item, done) {
    ...
    refresh = function(state) {
      ...
      if (state.show) {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          each = ref[j];
          results.push($show.append($("<p class=readout>" + each.readout + "</p>\n<p class=legend>" + each.legend + "</p>")));
        }
        return results;
      } else {
        ...
        if (output['debug']) {
          ...
          for (label in ref2) {
            value = ref2[label];
            results1.push(state.div.append($("<p class=error>" + label + " =><br> " + (inspect(value)) + "</p>")));
          }
          return results1;
        }
      }
    ...
    div.addClass('logic-source');
    div.get(0).logicValue = function() {
      return output.CF;
    };
