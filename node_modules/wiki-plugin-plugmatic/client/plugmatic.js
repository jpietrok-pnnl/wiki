(function() {
  ...
  traffic = function(installed, published) {
    var color;
    color = {
      gray: '#ccc',
      red: '#f55',
  ...
  expand = function(string) {
    ...
    external = function(match, href, protocol) {
      return stash("\"<a class=\"external\" target=\"_blank\" href=\"" + href + "\" title=\"" + href + "\" rel=\"nofollow\">" + (escape(href)) + "</a>\"");
    };
  ...
  emit = function($item, item) {
    ...
    render = function(data) {
      ...
      format = function(markup, plugin, dependencies) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          ...
          result.push((function() {
            ...
            switch (column) {
              ...
              case 'menu':
                return "<td title=menu> " + (((ref1 = plugin.factory) != null ? ref1.category : void 0) || '');
              case 'pages':
                return "<td title=pages style='text-align:center;'>" + (((ref2 = plugin.pages) != null ? ref2.length : void 0) || '');
              case 'service':
                return "<td title=service style='text-align:center;'>" + months;
              case 'bundled':
                return "<td title=bundled> " + (dependencies['wiki-plugin-' + name] || '');
              case 'installed':
                return "<td title=installed> " + (((ref3 = plugin["package"]) != null ? ref3.version : void 0) || '');
              case 'published':
                return "<td title=published> " + (((ref4 = pub(name).npm) != null ? ref4.version : void 0) || '');
            }
          })());
        }
        return result.join("\n");
      };
      report = function(markup, plugins, dependencies) {
        ...
        head = ((function() {
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            column = ref[i];
            results.push("<td style='font-size:75%; color:gray;'>" + column);
          }
          return results;
        })()).join("\n");
        result = ((function() {
        ...
        return "<center> <p><img src='/favicon.png' width=16> <span style='color:gray;'>" + window.location.host + "</span></p> <table style=\"width:100%;\"><tr> " + head + " " + result + "</table> <button class=restart>restart</button> </center>";
      };
      installer = function(row, npm) {
        ...
        choice = function(version) {
          var button, ref;
          button = function() {
            return "<button onclick=window.plugins.plugmatic.install('" + version + "')> install </button>";
          };
          return "<tr> <td> " + version + " <td> " + (version === ((ref = row["package"]) != null ? ref.version : void 0) ? 'installed' : button());
        };
        choices = (function() {
          var i, len, ref, results;
      ...
      detail = function(name, done) {
        ...
        pages = function(obj) {
          return "<p><b><a href=#>" + obj.title + "</a></b><br>" + obj.synopsis + "</p>";
        };
        birth = function(obj) {
          if (obj) {
        ...
        npmjs = function(more) {
          return $.getJSON("/plugin/plugmatic/view/" + name, more);
        };
        switch (column) {
          case 'status':
      ...
      showdetail = function(e) {
        ...
        return detail(name, function(html) {
          return wiki.dialog(name + " plugin " + column, html || '');
        });
      };
      more = function(e) {
        if (e.shiftKey) {
